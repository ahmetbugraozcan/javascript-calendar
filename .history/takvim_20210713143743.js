...
function fillCalendar() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (!isFromThisMonth(pazar[i])) {
            ...
            document.getElementById(`pazar${i}`).style.backgroundColor = "-backgroundcolor";
        }
        if (pazartesi[i] == currentDate.toLocaleDateString()) {

            document.getElementById(`pazartesi${i}`).style.color = "white";
        }
        if (sali[i] == currentDate.toLocaleDateString()) {

            document.getElementById(`sali${i}`).style.color = "white";
        }
        if (carsamba[i] == currentDate.toLocaleDateString()) {

            document.getElementById(`carsamba${i}`).style.color = "white";
        }
        if (persembe[i] == currentDate.toLocaleDateString()) {

            document.getElementById(`persembe${i}`).style.color = "white";
        }
        if (cuma[i] == currentDate.toLocaleDateString()) {

            document.getElementById(`cuma${i}`).style.color = "white";
        }
        if (cumartesi[i] == currentDate.toLocaleDateString()) {
...
function isFromThisMonth(Date) {
    console.log("tarihimiz " + Date(pazar[0]));
