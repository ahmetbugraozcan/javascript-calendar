//inputa gelen veri tıklananın verisi olabilirse süper olur. 
...
function getChildText(docID) {
    var doc = document.getElementById(docID);
    console.log("a : " + currentMonth);
    if(doc.hasChildNodes && (document.getElementById(docID).childNodes[1] != undefined)){
      //document.getElementById(docID).childNodes[1].textContent -> yapılacaklar textini dönüyor
...
function getChild(docID){
    document.getElementById(docID.childNodes[1]);
}
...
function operNotePad(text) {
    ...
    paragraf1.innerHTML = getChildText(text);
...
function addChildToCell(value, docName){
    ...
    document.getElementById(docName).chi
