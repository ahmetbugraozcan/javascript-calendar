...
function fillCalendarFunction(){
    ...
    for(var i = 0; i < databaseResponse["rows"].length; i++){
        ...
        //OLMADI BURA
        var ids =  databaseResponse["rows"][i]["id"];
        if(pazartesiKey[i] == databaseResponse["rows"][i]["id"]){
            addChildToCell(databaseResponse["rows"][i]["class"],`pazartesi${i}`)
        }else{
           
        }
        if(saliKey.includes(databaseResponse["rows"][i]["id"])){
            addChildToCell(databaseResponse["rows"][i]["class"],`sali${i}`)  
            console.log(databaseResponse["rows"][i]["id"])
            // document.getElementById(`saliKey${i}`).innerHTML = databaseResponse["rows"]["class"];
        }else{
            // console.log("key : " + pazartesiKey[i])
            // console.log("db  : " + databaseResponse["rows"][i]["id"])
        }
        if(carsambaKey[i] == databaseResponse["rows"][i]["id"]){
             addChildToCell(databaseResponse["rows"][i]["class"],`carsamba${i}`)
        }
        if(persembeKey[i] == databaseResponse["rows"][i]["id"]){
            addChildToCell(databaseResponse["rows"][i]["class"],`persembe${i}`)
        }
        if(cumaKey[i] == databaseResponse["rows"][i]["id"]){
            addChildToCell(databaseResponse["rows"][i]["class"],`cuma${i}`)
        }
        if(cumartesiKey[i] == databaseResponse["rows"][i]["id"]){
            addChildToCell(databaseResponse["rows"][i]["class"],`cumartesi${i}`)
        }
        if(pazarKey[i] == databaseResponse["rows"][i]["id"]){
