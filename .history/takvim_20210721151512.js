...
var today = new Date();
var defaultCellStyle = "font-weight:500;color:rgb(218, 80, 80);";
...
style.type = 'text/css';
style.innerHTML = `.cssClass { background-color: ${color};padding: 8px;
        color: white;
        letter-spacing: 3px;
        font-weight: bold;
        font-size: 17px;
    ...
    // document
    // .getElementsByClassName('days')[0].backgroundColor = getAverageRGB(image);
  };
};
...
function fillCalendarFunction(){
    setBackgroundImage();
    getColor();
    //  console.log(document.getElementById("calendar-header").style.backgroundImage.replace(/url\((['"])?(.*?)\1\)/gi, '$2')
    // .split(',')[0])
    
// var rgb = getAverageRGB(document.getElementById('calendar-header').style.backgroundImage);
