...
function fillCalendarFunction(){
    ...
    for(var i = 0; i < databaseResponse["rows"].length; i++){
        ...
        saliKey.push(`sali${j}` + "-" + currentYear + "-" + currentMonth )
            // var saliKey= `sali${j}` + "-" + currentYear + "-" + currentMonth ;
            var carsambaKey = `carsamba${j}` + "-" + currentYear + "-" + currentMonth ;
            var persembeKey = `persembe${j}` + "-" + currentYear + "-" + currentMonth ;
            var cumaKey = `cuma${j}` + "-" + currentYear + "-" + currentMonth ;
            var cumartesiKey = `cumartesi${j}` + "-" + currentYear + "-" + currentMonth ;
            var pazarKey =  `pazar${j}` + "-" + currentYear + "-" + currentMonth ;
        }
        console.log(saliKey)
        if( pazartesiKey.includes(databaseResponse["rows"][i]["id"])){
            addChildToCell(pazartesiKey,`pazartesi${i}`)
            // document.getElementById(`pazartesi${i}`).innerHTML = databaseResponse["rows"]["class"];
        }
        ...
        if(databaseResponse["rows"][i]["id"] == carsambaKey){
            document.getElementById(`carsamba${i}`).innerHTML = databaseResponse["rows"]["class"];
        }
        if(databaseResponse["rows"][i]["id"] == persembeKey){
            document.getElementById(`persembe${i}`).innerHTML = databaseResponse["rows"]["class"];
        }
        if(databaseResponse["rows"][i]["id"] == cumaKey){
            document.getElementById(`cuma${i}`).innerHTML = databaseResponse["rows"]["class"];
        }
        if(databaseResponse["rows"][i]["id"] == cumartesiKey){
            document.getElementById(`cumartesi${i}`).innerHTML = databaseResponse["rows"]["class"];
        }
        if(databaseResponse["rows"][i]["id"] == pazarKey){
            document.getElementById(`pazar${i}`).innerHTML = databaseResponse["rows"]["class"];
        }
