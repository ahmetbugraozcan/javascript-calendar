     //veriler güncellendikten sonra bir daha okunan dataları eşleştirmemiz gerekebilir
...
function fillCalendarFunction(){
    ...
    for(var i = 0; i < databaseResponse["rows"].length; i++){
      
        for(var j = 0; j < 6; j++){
            //her gün içn yapcaz diziye atıcaz
        pazartesiKey.push(`pazartesi${j}` + "-" + currentYear + "-" + currentMonth)
        saliKey.push(`sali${j}` + "-" + currentYear + "-" + currentMonth )
        carsambaKey.push(`carsamba${j}` + "-" + currentYear + "-" + currentMonth )
        persembeKey.push(`persembe${j}` + "-" + currentYear + "-" + currentMonth )
        cumaKey.push(`cuma${j}` + "-" + currentYear + "-" + currentMonth )
        cumartesiKey.push(`cumartesi${j}` + "-" + currentYear + "-" + currentMonth )
        pazarKey.push(`pazar${j}` + "-" + currentYear + "-" + currentMonth )
   
        }
        //OLMADI BURA
    
        if( pazartesiKey.includes(databaseResponse["rows"][i]["id"]) && databaseResponse["rows"][i]["id"] == `pazartesi${i}` + "-" + currentYear + "-" + currentMonth){
            addChildToCell(databaseResponse["rows"][i]["class"],`pazartesi${i}`)
        }
        if(saliKey.includes(databaseResponse["rows"][i]["id"]) && databaseResponse["rows"][i]["id"] == `sali${i}` + "-" + currentYear + "-" + currentMonth){
            addChildToCell(databaseResponse["rows"][i]["class"],`sali${i}`)  
            // document.getElementById(`saliKey${i}`).innerHTML = databaseResponse["rows"]["class"];
        }
        if(carsambaKey.includes(databaseResponse["rows"][i]["id"])){
             addChildToCell(databaseResponse["rows"][i]["class"],`carsamba${i}`)
        }
        if(persembeKey.includes(databaseResponse["rows"][i]["id"])){
            addChildToCell(databaseResponse["rows"][i]["class"],`persembe${i}`)
        }
        if(cumaKey.includes(databaseResponse["rows"][i]["id"])){
            addChildToCell(databaseResponse["rows"][i]["class"],`cuma${i}`)
        }
        if(cumartesiKey.includes(databaseResponse["rows"][i]["id"])){
            addChildToCell(databaseResponse["rows"][i]["class"],`cumartesi${i}`)
        }
        if(pazarKey.includes(databaseResponse["rows"][i]["id"])){
            addChildToCell(databaseResponse["rows"][i]["class"],`pazar${i}`)
        }
...
function removeChildNode(docName){

    var list = document.getElementById(docName);   
